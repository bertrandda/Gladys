...
class DiscoverTab extends Component {
  ...
  getDiscoveredDevices = async () => {
    ...
    try {
      const discoveredDevices = await this.props.httpClient.get('/api/v1/service/viessmann/discover');
      this.setState({
        discoveredDevices,
        loading: false,
        errorLoading: false
    ...
    return (
      <div class="card">
        <div class="card-header">
          <h1 class="card-title">
            <Text id="integration.viessmann.discover.title" />
          </h1>
          <div class="page-options d-flex">
            <button onClick={this.refreshDiscoveredDevices} class="btn btn-outline-primary ml-2" disabled={loading}>
              {!discoveredDevices && <Text id="integration.viessmann.discover.scan" />}
              {discoveredDevices && <Text id="integration.viessmann.discover.refresh" />}
              <i class="fe fe-radio" />
            </button>
          </div>
        </div>
        <div class="card-body">
          ...
          <div class="alert alert-secondary">
            <p>
              <Text id="integration.viessmann.discover.description" />
            </p>
            <p>
              <MarkupText id="integration.viessmann.discover.descriptionInformation" />
            </p>
          </div>
          <div
            class={cx('dimmer', {
